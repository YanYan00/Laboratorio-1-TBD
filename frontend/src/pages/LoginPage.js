...
import {
  ...
  Button, InputAdornment, Divider, Alert, CircularProgress, IconButton,
} from "@mui/material";
import {
...
const LoginPage = ({ onNavigate }) => {
  ...
  const validators = {
    identifier: (v) => v.trim().length >= 3 ? "" : "Ingresa tu usuario o email",
  ...
  const isValid = () =>
    !validators.identifier(form.identifier) && !validators.password(form.password);
  ...
  return (
    <Box sx={{ bgcolor: "#F5F7FB", minHeight: "100vh", display: "flex", alignItems: "center", py: 6 }}>
      <Container maxWidth="xs">
        ...
        <Paper elevation={0} sx={{
          border: "1px solid #E3E8F0", borderRadius: 3, p: { xs: 3, sm: 4 },
              ...
              <Box sx={{ textAlign: "center" }}>
                <Typography variant="caption" color="text.secondary" fontSize="0.8rem">
                  ...
                  <Button onClick={() => onNavigate("register")}
                    sx={{ textTransform: "none", color: "#1565C0", fontWeight: 600, fontSize: "0.8rem",
